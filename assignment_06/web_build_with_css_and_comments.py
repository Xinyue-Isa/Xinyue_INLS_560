import os # Import operating system module for file and path manipulation
import re # Import regular expression module for processing string formatting
...
def generate_nav(titles, active_title):
    ...
    nav_links = ""
    for title in titles:
        filename = slugify(title) # Get the slugify filename from title
        active_class = ' class="active"' if title == active_title else "" # Conditional addition of active class
        nav_links += f'\t\t\t<a href="{filename}"{active_class}>{title}</a>\n' # Building anchor tags with href
    return nav_links.strip() # Remove any leading or trailing blanks
...
def create_html_file(title, titles, output_dir="Build"):
    ...
    with open(output_path, 'w') as file:
        file.write(html_content) # Write HTML contents to files
...
def create_css_file(output_dir="build"):
    ...
    * {{
    ...
    nav {{
        background-color: {styles["nav-background"]}; # Set the background color of nav
        padding: 10px;
    }}
    ...
    nav a.active {{
        color: {styles["nav-a-active-color"]}; # Highlight the active nav link
        font-weight: bold;
    }}
    ...
    css_path = os.path.join(output_dir, "style.css") # Determine the full path of the CSS file
    with open(css_path, 'w') as file:
        file.write(css_content)

    print(f"Created style.css in the '{output_dir}' directory.") # Print and tell the user that CSS was created

# Main function.
def main():
    """Main function to generate pages and styles. MUST HAVE HOME!!!"""
    titles = ["Home", "About Us", "Shirts", "Suits"]
